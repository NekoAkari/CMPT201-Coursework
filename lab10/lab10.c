...
The client creates an IPv4 stream socket and connects to 127.0.0.1 on port 8001.
SOCK_STREAM used the TCP protocol.
The client sends each message using write().

2. What data is the client sending to the server?
The client sends five messages: "Hello", "Apple", "Car", "Green", "Dog".
Each message is stored in a 1024-byte buffer and sent to the server.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The run_acceptor thread receives a pointer to an acceptor_args structure,
It contains the run flag, a pointer to the shared linked list, and a pointer to the mutex that
protects the list.

2. How are received messages stored?
Each received message is copied into a dynamically allocated list node.
The nodes are stored in a shared links list.
A mutex protects the list when multiple client threads add messages.

3. What does `main()` do with the received messages?
main() waits until 20 messages have been received. It then stops the acceptor thread and waits for
it to finish.
Finally, it calls collect_all() to print and free every message, and checks that all messages were
collected.

4. How are threads used in this sample code?
main() creates one acceptor thread. The acceptor thread accepts client connections and created one
worker thread for each client.
Each worker thread reads messages from its client and adds them to the shared linked list.
The threads are stopped using run flags and cleaned up using pthread_join().

Non-blocking Sockets:
1. How are sockets made non-blocking?
The set_non_blocking() function uses fcntl() to get the current file status flags.
It than adds 0_NONBLOCK and sets the updated flags on the socket.

2. What sockets are made non-blocking?
The server listening socket and every connected client socket are made non-blocking.

3. Why are these sockets made non-blocking? What purpose does it serve?
A non-blocking socket dose not make accept() or read() wait forever when no connection or date is
available.
Instead, these functions return -1 with erron set to EAGAIN or EWOULDBLOCK.
This allows each thread to continue checking its run flag and stop cleanly when the flag becomes
false.
...
static void *run_acceptor(void *args) {
  ...
  // Wait until all client threads have stopped
  for (int i = 0; i < num_clients; i++) {
...
int main() {
  ...
  while (true) {
    ...
    bool enough_messages = list_handle.count >= MAX_CLIENTS * NUM_MSG_PER_CLIENT;
